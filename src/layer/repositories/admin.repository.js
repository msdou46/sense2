...
class AdminRepository {
  ...
  update_to_admin = async (user_id, admin_type) => {
    return await this.adminModels.update({ type : admin_type }, { where: { user_id } })
  }

  find_all_lectures = async () => {
    return await this.adminModels.findAll()
  }
  update_lecture = async (lecture_id, lecturer, title, content, category, image, point) => {
  ...
  create_lecture = async (lecturer, title, content, category, image, point) => {
      return await this.adminModels.create({lecturer, title, content, category, image, point})
    }
}
